        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <flux:button type="button" variant="ghost" wire:click="generateImage" wire:loading.attr="disabled" wire:target="generateImage">
                    <span wire:loading.remove wire:target="generateImage">{{ $product?->image_path ? 'Regenerar imagen con IA' : 'Generar imagen con IA' }}</span>
                ...
                <flux:text size="sm">La imagen se genera directo con el nombre y la descripción actuales.</flux:text>
            </div>
            @if ($image)
